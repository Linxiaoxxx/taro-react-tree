...
const CanvasDrawer: React.FC<CanvasDrawerProps> = ({ config, showLoading, onCreateSuccess, onCreateFail }) => {
  const { width, height, backgroundColor, texts = [], blocks = [], lines = [], debug = false } = config || {}
  ...
  return (
    <Canvas type='2d' id={canvasId} style={`height: ${height}rpx; width: ${width}rpx; position: absolute;${debug ? '' : 'transform: translate3d(-9999rpx, 0, 0)'}`} />
  )
}
